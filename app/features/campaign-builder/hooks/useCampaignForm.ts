...
export const useCampaignForm = () => {
  ...
  const validateCrossFields = useCallback((data: CampaignFormInput): Record<string, string> => {
    ...
    try {
      ...
      const diffTime = Math.abs(
        transformedData.endDate.getTime() - transformedData.startDate.getTime()
      )
  ...
  const handleBackendErrors = useCallback(
    (backendErrors: { field: string; message: string }[]) => {
  ...
  const validateBeforeSubmit = useCallback(
    (data: CampaignFormInput): boolean => {
