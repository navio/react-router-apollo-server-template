...
} from '../services/internal-queries'
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: '1rem' }}>
        ...
        />
        <h3 style={{ margin: 0, fontSize: '1.25rem', fontWeight: '600' }}>Server Status</h3>
      </div>
  ...
  return (
    ...
    >
      <div style={{ padding: '1.5rem', borderBottom: '1px solid #e2e8f0' }}>
        <h3 style={{ margin: 0, fontSize: '1.25rem', fontWeight: '600' }}>Internal Data</h3>
      </div>
      ...
      <div style={{ padding: '1rem' }}>
        {data.map((item) => (
          ...
          >
            ...
            <div style={{ color: '#6b7280', marginBottom: '0.25rem' }}>Value: {item.value}</div>
...
function CreateDataForm() {
  ...
  return (
    ...
    >
      ...
      {successMessage && (
        <div style={{ color: '#10b981', marginTop: '0.5rem' }}>{successMessage}</div>
      )}
    </div>
  )
}
...
export default function Internal() {
  ...
  const error = useInternalError()
  const { clearError, setHealth, setInternalData, setHealthLoading, setDataLoading } =
    useInternalStore()
  ...
  useEffect(() => {
    ...
    const unsubscribeDataUpdated = on('DATA_UPDATED', (event) => {
      if (
        ...
        event.payload.type === 'internal-data-created'
      ) {
      ...
      >
        ...
        <div style={{ display: 'flex', gap: '1rem' }}>
          {(healthLoading || dataLoading) && (
            <div style={{ color: '#6b7280', fontSize: '0.875rem' }}>Updating...</div>
          )}
          <button
            onClick={() => {
              fetchHealth()
